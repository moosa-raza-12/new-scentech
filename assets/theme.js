...
document.addEventListener('DOMContentLoaded', function() {
    ...
    links.forEach(link => {
        // Check if the link points to the specific URLs
        if (link.href === "https://getscentech.com/pages/find-a-scent-that-matches-your-personality" || 
            link.href === "https://getscentech.com/pages/find-a-scent-that-fits-your-business") {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('scroll', function () {
    ...
    if (scrollY > 100) { // Toon de knop na 100px scrollen
...
document.addEventListener("DOMContentLoaded", function () {
    const checkElements = setInterval(function () {
        ...
        if (stickyCTAButton && gorgiasChat) {
            gorgiasChat.style.display = "none";
            clearInterval(checkElements); // Stop checking once both elements are found
        }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const scentSlug = urlParams.get('scent');

// PART 1: Redirect from quiz result product to bundle
if (
  source === 'lantern' &&
  (
    campaign === 'nl-find-a-fragrance-that-fits-your-personality-redirect-get-started-bundle' ||
    campaign === 'find-a-fragrance-that-fits-your-personality-redirect-get-started-bundle'
  ) &&
  pathname.startsWith('/products/') &&
  pathname !== '/products/get-started-bundle-new'
) {
  const productSlug = pathname.split('/products/')[1].replace(/\/$/, '');
  window.location.href = `/products/get-started-bundle-new?scent=${productSlug}`;
  return;
}


  // PART 2: Auto-select scent in the bundle page
  if (pathname === '/products/get-started-bundle-new' && scentSlug) {
    // Converts 'pure-nirvana' → 'Pure Nirvana'
    const normalizeScentName = (slug) => {
      return slug
        .split('-')
    ...
    const targetScentName = normalizeScentName(scentSlug);

    const trySelectScent = (attempt = 0) => {
      const inputs = document.querySelectorAll('input[name="properties[Kies je geur]"]');

      if (inputs.length === 0) {
        if (attempt < 10) {
          setTimeout(() => trySelectScent(attempt + 1), 300);
        }
        return;
      }

      const input = Array.from(inputs).find(input =>
        input.value.trim().toLowerCase() === targetScentName.toLowerCase()
      );
      ...
      if (!input.checked) {
        console.log(`Auto-selecting scent: "${input.value}" (attempt ${attempt + 1})`);
        input.checked = true;
    ...
    // Start selection after a short delay
    setTimeout(() => trySelectScent(), 1000);
  }
});
